...
export default {
  ...
  computed: {
    device () {
      return client.device || undefined
    }
  },

  methods: {
    async executeShellCommand (device, command) {
    ...
    async getDeviceInfo () {
      ...
      this.deviceInfo = {
        ...
        resolution: await this.executeShellCommand(adbDevice, "wm size | awk '{print $3}'"),
        screenDensity: await adbDevice.getProp('ro.sf.lcd_density'),
        ipAddress: await this.executeShellCommand(
          adbDevice,
          "ip addr show wlan0 | grep 'inet ' | cut -d' ' -f6 | cut -d/ -f1"
        ),
        totalMemory: `${await this.executeShellCommand(
          adbDevice,
          "free -m | awk '/Mem:/ {print $2}'"
        )} MB`,
        usedMemory: await this.executeShellCommand(
          adbDevice,
          "free -m | awk '/Mem:/ {print $3}'"
        ),
        totalStorage: await this.executeShellCommand(
          adbDevice,
          "df -h /data | awk '/\\/data/ {print $2}'"
        ),
        usedStorage: await this.executeShellCommand(
          adbDevice,
          "df -h /data | awk '/\\/data/ {print $3}'"
        ),
        serialNumber: await adbDevice.getProp('ro.serialno'),
        cpuInfo: await adbDevice.getProp('ro.hardware'),
        cpuMin: await this.executeShellCommand(
          adbDevice,
          'cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_min_freq'
        ),
        cpuMax: await this.executeShellCommand(
          adbDevice,
          'cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq'
        ),
        cpuCur: await this.executeShellCommand(
          adbDevice,
        ...
        batteryPercentage: parseInt(await this.executeShellCommand(adbDevice, 'dumpsys battery | grep level | awk \'{print $2}\''), 10),
        voltage: parseFloat((await this.executeShellCommand(
          adbDevice,
          'dumpsys battery | grep voltage | awk \'{print $2}\''
        )) || '0') / 1000,
        temperature: parseInt(await this.executeShellCommand(adbDevice, 'dumpsys battery | grep temperature | awk \'{print $2}\''), 10) / 10,
        bootloader: await this.executeShellCommand(
          adbDevice,
          'getprop ro.boot.verifiedbootstate'
        ),
        abPartition: await this.executeShellCommand(
          adbDevice,
          'getprop ro.boot.slot_suffix'
        ),
        uptime: await this.executeShellCommand(
          adbDevice,
          'cat /proc/uptime | cut -d. -f1'
        ),
        storageType: await this.executeShellCommand(
          adbDevice,
          'getprop ro.boot.bootdevice'
        ),
        kernelVersion: await this.executeShellCommand(
          adbDevice,
          'uname -r'
        )
      }
    ...
    async openSettings () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.SETTINGS')
    },

    async openDeveloperOptions () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.APPLICATION_DEVELOPMENT_SETTINGS')
    },

    async openBrowser () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.intent.action.VIEW -d "http://www.google.com"')
    },

    async openWifiSettings () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.WIFI_SETTINGS')
    },

    async openDisplaySettings () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.DISPLAY_SETTINGS')
    },

    async openSoundSettings () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.SOUND_SETTINGS')
    },

    async openAppSettings () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.APPLICATION_SETTINGS')
    },

    async openAboutPhone () {
      if (!this.device) return
      await this.executeShellCommand(this.device, 'am start -a android.settings.DEVICE_INFO_SETTINGS')
    },
        ...
        <div v-if="isLoading" :key="'loading'" class="info-grid">
          <div class="info-main">
            <v-skeleton-loader
              ...
              class="skeleton-card"></v-skeleton-loader>
          </div>
          <div class="info-side">
            <v-skeleton-loader
              type="card"
